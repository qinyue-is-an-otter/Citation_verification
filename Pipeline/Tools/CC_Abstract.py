...
def seen_sent(seen, sentences, citation_text, rid, data, ref_dict, ref_id, full_text, citing_doi, pmcid):  
    for i,s in enumerate(sentences):
        if (citation_text and citation_text in s) or rid in s:
            if len(s) < 125: # the max length of the citation context is to be determined
                sent = ' '.join(sentences[max(i-1,0): min(len(sentences),i +1)])
                sent = re.sub(r'[^a-zA-Z0-9., ]', '', sent, flags=re.ASCII)
                matching_sentences = [sent]
            else:
                matching_sentences = [s]
...
def doi_to_pmcid(dois):
    ...
    result_df.to_csv('Intermediate/doi_to_pmcids.tsv', index=False, sep='\t')
...
def read_metadata(input_data):
    """
    Reads DOI from various formats: csv, tsv, txt files or a list of dois
    """
    lis = re.split(',', input_data)
        #print(lis)
    return lis  
    """  if isinstance(input_data, str)and any(input_data.endswith(ext) for ext in ['.csv', '.tsv', '.txt']):  # File path
        try:
            # Try CSV/TSV 
            return pd.read_csv(input_data, dtype=str, engine='python', sep=None)
        except pd.errors.ParserError:
            # txt
            with open(input_data, 'r') as f:
                lines = [line.strip().split('\t') for line in f.readlines()]
                df = pd.DataFrame(lines[1:], columns=lines[0])
            return df['DOI'].dropna().unique()
    else:
        
    raise ValueError("Unsupported input format. Provide file path, list, or DataFrame.")"""
             
"""########## Main Function #############"""

if __name__ == "__main__":
    #input_data = input('Pls Enter the path to the dois file Or give the list of dois: ')
    #dois = read_metadata(input_data)
    df = pd.read_csv('test.csv', dtype=str)
    dois = df['DOI'].dropna().unique()
    start = time.time()
    ids = doi_to_pmcid(dois)
    #print(ids)
    #print(len(ids[ids.PMID != 'Not Found']), len(ids[ids.PMCID != 'Not Found']),len(ids['DOI'].dropna()))

    pmcid = ids[ids.PMCID != 'Not Found']
    #print(pmcid.PMCID)
    #p = pmcid.PMCID.head(5)
    count = fetch_pubmed_articles(pmcid.PMCID)
    print(f'{count} xml files were downloaded from PMC')
    logger.info(f'{count} xml files were downloaded from PMC')
    pmc_citations_df = process_files(path)
    pmc_citations_df.to_csv('Intermediate/pmc_cc.tsv', sep = '\t', index=False) 
    if not pmc_citations_df.empty:
        unique_dois = pmc_citations_df["DOI"].unique()
        abstract(unique_dois,pmc_citations_df,ids)
